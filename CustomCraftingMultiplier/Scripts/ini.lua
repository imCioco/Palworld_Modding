...
-- Windows C locales can case-fold individual UTF-8 bytes. Fold ASCII only.
function Ini.key(s)
        ...
        if line == "" or first == ";" or first == "#" then
            ...
            if name then
                ...
                if key then
                    ...
                    data[section][Ini.key(trim(key))] = value
                end
            end
        end
    end
...
local function raw(data, section, key)
    ...
    local s = data[Ini.key(section)]
    if type(s) ~= "table" then return nil end
    return s[Ini.key(key)]
end
